...
/**
 * Run the incremental sample scanner using SdkTestUtils and test that the correct documents are published.
 *
 */
public class SampleIncrementalDataSourceScannerTest
{
  ...
  private void doTest(List<String> fileList) throws Exception {
    ...
    try {
      ...
      SampleIncrementalDataSourceScanner sampleScanner = new SampleIncrementalDataSourceScanner();
      sampleScanner.setFileList(scannerFileList);
      sampleScanner.setIncrementalDeletes(false); // Existing documents will not be deleted if not seen during the scan
      sampleScanner.setIncrementalModeActivated(true); // Only new or modified documents will be published
      ...
      TestScannerRunner scannerRunner = SdkTestUtils.createTestScannerRunner("sampleIncrementalScannerConnector", sampleScanner);
      ...
      Assert.assertEquals("Expected no documents since it's an incremental scanner and the data source was not modified", 0, documentList.size());
      ...
      scannerRunner.start();
      documentList = (DocumentList) scannerRunner.getSentMessages().get(0);
      Assert.assertEquals("Wrong number of documents - all the documents should be there after reset", FILE_COUNT, documentList.size());
  ...
  private void validateDocument(DocumentList documentList, String... text) {
    ...
    for (IngestDocument doc : documentList) {
      ...
      Assert.assertNotNull("An ACL must be attached to the document since the scanner was configured for document security", doc.getAcl());      
      for (String stringItem : text)
        Assert.assertTrue("Expected text is missing", doc.getFirstValue(FieldNames.TEXT).stringValue().contains(stringItem));
    }
  }
}
