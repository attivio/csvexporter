...
public class SampleDataSourceScannerUITest
{
  ...
  //Note that the _s postfix of the field name create a dynamic field - it doen't have to be defines statically in the schema to be indexed
  ...
  public void test() throws AttivioException {
    ...
    TestScannerRunner scannerRunner = SdkTestUtils.createTestScannerRunner("sampleScannerConnector", sampleScanner);
    scannerRunner.start();
  ...
  private void validateItem(IngestDocument doc, String fieldName) {
    Assert.assertTrue(doc.getField(fieldName).getValue(0).stringValue().equals(ITEM_MAP.get(fieldName)));
  }
