...
/**
 * Tests the {@link SampleHttpDataSourceScanner}. The test retrieves the content from the Google web site url and sets as text in the
 * Attivio document. Since the Google site does not require authentication the user/password and the other {@link BasicAuthentication}
 * parameters are not actually used. To actually test Basic authentication, this test can be modified by replacing the Google url with a url of
 * a site that is protected by Http Basic authentication. Then, modify the {@code configureBasicAuthentication} method to set the
 * correct parameters and observe the {@link FieldNames.TEXT} value in the {@link IngestDocument}.
 */
public class SampleHttpDataSourceScannerTest
{
  ...
  public void test() throws AttivioException {
    ...
    TestScannerRunner scannerRunner = SdkTestUtils.createTestScannerRunner("testConnector", scanner);
    ...
    for(IngestDocument doc : documentList) {
      if (doc.getFirstValue(FieldNames.TEXT).stringValue().contains(TEXT_IN_GOOGLE_WEBSITE))
        return;
    }
