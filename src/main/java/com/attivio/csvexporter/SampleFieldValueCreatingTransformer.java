...
/**
 * Sample document transformer demonstrating implementation of a FieldValueCreatingTransformer. In this example, a new field value
 * is created that will contain the lower-case version of the input field's value.
 */
...
public class SampleFieldValueCreatingTransformer implements FieldValueCreatingTransformer<String> {
  ...
  public IngestFieldValue createMappedValue(String inputFieldName, IngestFieldValue fv) throws AttivioException {
    if (fv.getValue() instanceof String) {
      ...
      throw new AttivioException(IndexWorkflowError.INVALID_FIELD_TYPE, "Field %s is not a String field", inputFieldName);
    }
  }
