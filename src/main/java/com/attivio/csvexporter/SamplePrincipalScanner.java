...
@ConfigurationOptionInfo(
    ...
    description="Demonstrates the reading of principal info from files and loading into the Attivio index",
    groups= {
...
public class SamplePrincipalScanner implements DataSourceScanner {
  ...
  public void start(String connectorName, DocumentPublisher publisher) throws AttivioException {
    try {
      ...
      if (e instanceof AttivioException)
        throw (AttivioException) e;
      else
        throw new AttivioException(ConnectorError.CRAWL_FAILED, e, "Connector %s run failed", connectorName);
    }
  }
  ...
  private AttivioPrincipal createPrincipal(String line) {
    ...
    PrincipalType pType = tokenizer.nextToken().equals("user") ? PrincipalType.USER : PrincipalType.GROUP;
  ...
  private void addMemebership(AttivioPrincipal group, String memberRealm, String memberId) {
    AttivioPrincipalKey groupKey = new AttivioPrincipalKey(group.getRealmId(), group.getPrincipalId());
  ...
  @ConfigurationOption(longOpt = "principals-file", displayName = "Principals File", description = "The path to a CSV file the contains users and groups")
  ...
  @ConfigurationOption(longOpt = "memberships-file", displayName = "Memberships File", description = "The path to a CSV file the contains membership associations")
  ...
  public void validateConfiguration() throws AttivioException {
    if (principalsFile != null && !principalsFile.exists())
      throw new AttivioException(ConnectorError.CONFIGURATION_ERROR, "%s doesn't exist", principalsFile.getAbsolutePath());

    if (membershipsFile != null && !membershipsFile.exists())
      throw new AttivioException(ConnectorError.CONFIGURATION_ERROR, "%s doesn't exist", membershipsFile.getAbsolutePath());
  }
