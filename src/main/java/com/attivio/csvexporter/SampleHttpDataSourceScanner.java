...
/**
 * This sample scanner demonstrates how to use the {@link HttpDataSourceScanner} interface to take advantage of the Attivio provided implementations of Http client authentication protocols. in this
 * example, the scanner implements the {@link BasicAuthentication} interface. Once that interface is implemented, Attivio will create and configure for the scanner a {@link HttpClientProvider} object.
 * If the scanner implements more than a single authentication interface, say {@link BasicAuthentication} and {@link DigestAuthentication}, Attivio will configure the one that was configured. That
 * allows the scanner to support multiple Http protocols.
 * 
 */
@ScannerInfo(suggestedWorkflow = "ingest") //Use fileIngest for large documents and documents with complex formats such as PDF files
@ConfigurationOptionInfo(
    ...
    groups= {
        @ConfigurationOptionInfo.Group(path={ConfigurationOptionInfo.SCANNER, "BasicAuthentication"}, 
                                       propertyNames={"basicAuthLogin","basicAuthPassword","basicAuthRealm","basicAuthPort","basicAuthHost"}),
        @ConfigurationOptionInfo.Group(path={ConfigurationOptionInfo.SCANNER, "HttpConnConfig"}, 
                                       propertyNames={"maxConnTotal","maxConnPerRoute"}),
        @ConfigurationOptionInfo.Group(path=ConfigurationOptionInfo.SCANNER ,propertyNames={"contentUrl"})
    }
)
public class SampleHttpDataSourceScanner implements DataSourceScanner, HttpDataSourceScanner, BasicAuthentication, HttpConnectionConfig {
  ...
  public void start(String connectorName, DocumentPublisher publisher) throws AttivioException {
    try {
      ...
      CloseableHttpClient httpClient = httpProvider.prapereHttpClient(contentUrl);
      CloseableHttpResponse response = httpClient.execute(get);
      int status = response.getStatusLine().getStatusCode();
      if (response.getStatusLine().getStatusCode() != 200)
        throw new AttivioException(ConnectorError.CRAWL_FAILED, "Didn't get OK status from %s - %s", contentUrl, status);
      ...
      throw new AttivioException(ConnectorError.CRAWL_FAILED, e, "Failed to retrieve content from %s", contentUrl);
    } finally {
      if (httpProvider != null)
        IOUtils.closeQuietly(httpProvider);
    }
  }
