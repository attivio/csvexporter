...
/**
 * An example for configuring a scanner with lists and maps. The configured values are added to a document to demonstrate multiple field values
 * and dynamic fields.
 */
...
@ConfigurationOptionInfo(
    ...
    description="An example for list and map configuration items, multiple value fiedls and dynamic fields",
        
    groups= {
        @ConfigurationOptionInfo.Group(path=ConfigurationOptionInfo.SCANNER ,propertyNames={"listItems","mapItems"})
    }
)
public class SampleDataSourceScannerUI implements DataSourceScanner {
  ...
  @ConfigurationOption(longOpt = "list-items", displayName = "List Items", description = "A list of items to be put in an Attivio document field", formEntryClass = ConfigurationOption.STRING_LIST)
  ...
  @ConfigurationOption(longOpt = "map-items", displayName = "Map Items", description = "A field-value map to be put in the Attivio document", formEntryClass = ConfigurationOption.STRING_TO_STRING_MAP)
  ...
  @ConfigurationOption(longOpt = "list-field", displayName = "List Field", description = "A field to populate with list items")
  ...
  public void validateConfiguration() throws AttivioException {
    if (listField == null)
      throw new AttivioException(ConnectorError.CONFIGURATION_ERROR, "List field was not configured");
  }
